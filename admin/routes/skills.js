...
router.get('/:id', (req, res) => {
  const skill = get('SELECT * FROM skills WHERE id = ?', [req.params.id]);
...
router.post('/', (req, res) => {
  ...
  res.status(201).json(get('SELECT * FROM skills WHERE id = ?', [id]));
});

router.put('/:id', (req, res) => {
  const existing = get('SELECT * FROM skills WHERE id = ?', [req.params.id]);
  ...
  res.json(get('SELECT * FROM skills WHERE id = ?', [req.params.id]));
});
